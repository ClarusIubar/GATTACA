...
- **스토리지 엔진 주입 (Dependency Injection)**: 런타임 시 구동 모드(데모 모드, Cloudflare 에지 모드)에 따라 알맞은 리포지토리 인스턴스(`DemoRepository` 또는 `CloudflareRepository`)를 생성하여 `AppProvider` 컨텍스트에 주입하므로 프론트엔드 변경 없이 데이터 레이어를 원천 스위칭할 수 있습니다.
...
> [!IMPORTANT]
> 本 권한 가드는 `app-context.tsx` 내부 정책 게이트에서 프론트엔드 방어막으로 1차 작동하며, 실제 배포 시에는 Cloudflare D1 SQL 데이터 보안 및 Workers API 게이트웨이 정책이 에지 단에서 2차로 강제하여 데이터 조작 시도를 원천 봉쇄합니다.
