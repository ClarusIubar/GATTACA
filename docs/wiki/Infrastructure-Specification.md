...
### 4.2 GitHub Actions (CI/CD) Secrets 및 Variables 등록
코드 푸시 시 자동으로 Pages 정적 빌드 및 배포가 수행되도록 GitHub 레포지토리에 보안 토큰과 일반 환경 변수를 이원화 등록하는 방법입니다.

#### 1. Repository Secrets (비밀 보안 키 - 단 2개만 등록!)
GitHub의 Secrets는 외부에 결코 노출되어서는 안 되는 비밀자격증명(배포 권한 키) 전용 영역입니다.
1. 본인의 GitHub `GATTACA` 저장소 웹 페이지 ➡️ **[Settings]** ➡️ **[Secrets and variables]** ➡️ **[Actions]**로 이동합니다.
2. **[New repository secret]**을 클릭해 아래 **단 2개의 비밀키**만 생성/등록합니다:
   - **Name**: `CLOUDFLARE_API_TOKEN`
     - **Value**: Cloudflare API Token 문자열 붙여넣기 (배포 러너 인증용)
   - **Name**: `CLOUDFLARE_ACCOUNT_ID`
     - **Value**: 32자리 Account ID 문자열 붙여넣기 (배포 계정 식별용)

#### 2. Repository Variables (비민감 빌드 설정 변수 - vars 등록)
비밀번호가 아닌 일반적인 서버 URL 주소나 환경 변수 등은 Secrets가 아닌 Variables 탭에서 편리하게 관리합니다.
1. Actions 페이지 내의 상단 탭에서 **[Variables]** (Secrets 우측 탭)를 클릭합니다.
2. **[New repository variable]**을 클릭해 아래 변수들을 등록합니다 (Vite 클라이언트 전용 접두사 `VITE_`가 생략된 이름으로 등록하면 빌드 파이프라인에서 자동으로 매핑 주입합니다):
   - **Name**: `CLOUDFLARE_API_URL`
     - **Value**: Cloudflare Workers 백엔드 API Gateway 주소 (`https://gattaca-backend.your-subdomain.workers.dev`) 등록
   - **Name**: `ADMIN_USER_ID`
     - **Value**: 운영자 사용자 고유 ID 등록
...
### 5.2 CI/CD 통합 배포 경계 (GitHub Secrets & Variables 원스톱 무인 빌드/배포)
- **비밀 키(Secrets)와 일반 설정 변수(Variables)의 명확한 경계**:
  ...
  - 하지만 이제 **비밀 배포 토큰은 GitHub Secrets에, 빌드 타임 설정 주소(`CLOUDFLARE_API_URL`, `ADMIN_USER_ID`)는 GitHub Variables에 완벽히 이원화하여 등록**함으로써 파이프라인의 물리 보안 정합성을 극대화하였습니다.
  - **VITE 접두사 배제 정책**: 깃허브 저장소 Variables에 등록되는 설정 값들은 클라이언트 프레임워크나 특정 빌드 도구(Vite)에 종속되지 않는 순수한 인프라 리소스 값으로 보관되어야 하므로, 접두사 `VITE_`를 제거한 표준 이름(`CLOUDFLARE_API_URL`, `ADMIN_USER_ID`)으로 깨끗하게 보관합니다.
  - 배포 파이프라인(GitHub Actions)은 빌드 단계(`env`)에서 해당 Variables를 끌어와 Vite 번들러가 컴파일 인라이닝할 수 있도록 접두사를 매핑 주입(`VITE_CLOUDFLARE_API_URL: ${{ vars.CLOUDFLARE_API_URL }}`)해줌으로써 최종 정적 에셋 번들 내부에 안전하게 인라이닝 처리합니다.
- **런타임 컨테이너 환경 변수와 빌드 타임 변수의 경계**:
  - React SPA 정적 에셋 빌드에 필요한 변수는 GitHub Actions 파이프라인 시점에 GitHub Variables에서 꺼내어 번들에 즉시 주입(Inlining)되므로, 번거롭고 실수하기 쉬운 Cloudflare Dashboard 수동 설정을 완전히 배제할 수 있습니다.
