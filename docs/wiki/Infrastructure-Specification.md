...
- **GitHub Secrets 기반의 통합 주입 (Single Source of Truth)**:
  ...
  - 하지만 이제 **GitHub Secrets에 Cloudflare 배포 키와 빌드 타임 환경 변수(`VITE_CLOUDFLARE_API_URL`)를 통합 등록**하여 완전 무인화된 CI/CD 파이프라인을 구성합니다.
  - 이로 인해 GitHub Actions가 코드가 푸시되는 시점에 `VITE_CLOUDFLARE_API_URL`을 빌드 컴파일 타임에 React 자바스크립트 번들에 인라이닝 주입하고, 산출물(`dist`)을 획득한 배포 토큰 권한으로 Cloudflare Pages 글로벌 CDN에 한 번에 배포 및 이식합니다.
