...
describe('Memory Train end-to-end user flow', () => {
  ...
  it('creates a station with separate date and time inputs, opens detail, adds a memory, and adds a comment', async () => {
    ...
    expect(screen.getByText('가벼운 파티와 사진 전시')).toBeInTheDocument()

    await user.type(screen.getByLabelText('메모리 캡션'), '첫 번째 강남역 단체 사진')
    ...
    expect(await screen.findByText('첫 번째 강남역 단체 사진')).toBeInTheDocument()

    await user.type(screen.getByLabelText('코멘트 남기기'), '비 오던 날이라 더 기억에 남아요')
    await user.click(screen.getByRole('button', { name: '코멘트 추가' }))

    expect(await screen.findByText('비 오던 날이라 더 기억에 남아요')).toBeInTheDocument()
  })
})
